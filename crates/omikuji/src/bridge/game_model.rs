...
impl qobject::GameModel {
    ...
    fn launch_game_force(&self, index: i32) -> bool {
        let idx = index as usize;
        let Some(game) = self.library.game.get(idx) else {
    ...
    fn check_epic_update(&self, game_id: &QString) -> bool {
    ...
    fn check_gog_update(&self, game_id: &QString) -> bool {
        let gid = game_id.to_string();
        let Some(game) = self.library.game.iter().find(|g| g.metadata.id == gid) else {
            return false;
        };
    ...
    fn scan_all_for_updates(mut self: Pin<&mut Self>) {
