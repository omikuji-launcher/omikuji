...
DialogCard {
    ...
    function _existing(app, path) {
        let out = {}
        try { out = JSON.parse(gameModel.nile_check_existing_install(app, path) || "{}") || {} }
        catch (e) { out = {} }
        return out
    }
    ...
    function refreshExistingInstall() {
        ...
        if (!isImportMode && rawPath !== "" && gameModel.nile_dir_has_game(gameData.appName, rawPath)) {
    ...
    function refreshInstallSize() {
        if (!gameModel || !gameData || !gameData.appName) {
            installBytes = -1; sizeError = ""; return
        }
        installBytes = -2
        sizeError = ""
        let id = "nile-" + Date.now().toString(36) + "-" + Math.random().toString(36).substring(2, 8)
        _sizeRequestId = id
        gameModel.fetch_nile_install_size(id, gameData.appName)
    }
    ...
    function show() {
        if (!nileModel || gameIndex < 0) return
        resetState()
        gameData = nileModel.get_game_at(gameIndex)
        let gameDir = gameData && gameData.installPath ? gameData.installPath : ""
        if (gameData && gameData.isInstalled === true && gameDir !== "") {
    ...
    body: ColumnLayout {
        ...
        NoteChip {
            ...
            text: qsTr("nile allows only one install per game. To reinstall elsewhere, delete the game files first.")
        }
        ...
        M3Button {
            ...
            onClicked: {
                let runner = root.runnerOptions.length > 0
                    ...
                    : ""
                let id = root.nileModel.enqueue_install(
                    root.gameIndex,
