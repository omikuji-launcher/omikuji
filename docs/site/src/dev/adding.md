...
A setting is a value in `app.toml` that QML reads and writes live. The bridge for it is generated by [kushi](https://github.com/reakjra/kushi) from a declaration in `crates/omikuji/build.rs`. For a new bool `show_clock`:
